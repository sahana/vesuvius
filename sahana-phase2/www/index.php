...
function shn_main_front_controller()
{
	...
	if( array_key_exists('stream', $_REQUEST) && 
        ...
        if (array_key_exists('stream', $_REQUEST)) {
            add_error(_('The stream requested is not a valid stream'));
        }
	...
	if (file_exists($module_file)) {
        ...
        add_error(_('The requested module is not installed in Sahana'));
        $module = 'home';
        $action = 'default';
    ...
    if (!function_exists($module_function)) {
        ...
        if (!function_exists($module_function)) {
            ...
            if ( null == $stream_ ) {
                add_error(_('The action requested is not available'));
                $module_function = 'shn_'.$module.'_default';
            } else {
                // if this does not exist display the error in the html homepage
                add_error(_('This action does not support the '.$stream.' stream type.'));
