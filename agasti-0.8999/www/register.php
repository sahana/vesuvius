...
function confirm() {
	...
	$query  = "
		SELECT * 
		FROM reg 
		WHERE API_KEY = '".mysql_real_escape_string($api)."';
	";
...
/**
 * Check for valid confirmation code for the newPassword procedure
 */
function newPassword() {
	...
	$query  = "
		...
		AND confirmation_code = '".mysql_real_escape_string($confirmation_code)."' ;
	";
	...
	if($row = $result->FetchRow()) {
		...
		$bodyHTML = 
			"Your new password is <b>".$new_pass."</b><br><br>Use it at your next login. You may change is from the user preferences page once logged in.<br><br>";
		$bodyAlt = 
			"Your new password is ".$new_pass." \n\nUse it at your next login. You may change is from the user preferences page once logged in.\n\n";
...
function addUser($email, $password, $name, $p_uuid) {
	...
	$ret = shn_auth_add_user($fisrtName, $lastName, $email, $password, $role=REGISTERED, $p_uuid);
}
...
/**
 * Passed the user along to a page in the module reporting password change failure.
 */
function changePasswordFail() {
	header("Location: ".makeBaseUrl()."index.php?mod=rez&act=default&page_id=-5");
}



/**
 * Passed the user along to a page in the module reporting password change success.
 */
function changePasswordWin() {
	header("Location: ".makeBaseUrl()."index.php?mod=rez&act=default&page_id=-6");
}
...
*/
	...
	$bodyHTML = 
		...
		."Afterwards, you will also be able to login to the Person Locator web site by going here and logging in with the following credentials: <br><br>"
	...
	$bodyAlt = 
		...
		."Afterwards, you will also be able to login to the Person Locator web site by going here and logging in with the following credentials:\n\n"
		.makeBaseUrl()."\n"
		."Username: ".$email."\nPassword: ".$password."\n\n"
