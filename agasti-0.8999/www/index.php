...
// uncomment the two lines below if you are using internal Agasti feedback system
//require_once ($APPROOT.'inc/user_feedback.inc');
//$global['user_feedback_enabled'] = true;
...
function shn_main_filter_getpost() {
	...
	if (!$global['previous']) {
		...
		if ((($global['action'] == 'signup') || ($global['action'] == 'forgotPassword')) && ($_REQUEST['mod'] == null)) {
			$global['module'] = "pref";
		}
	}
}
...
function shn_main_front_controller() {
	...
	// check if we are enabling ACL
	if((isset($_GET['mod'])) && (isset($_GET['act'])) && ($_GET["mod"] == "admin") && ($_GET["act"] == "acl_enable_acl_cr")) {
	...
	// are we streaming PLS Services? -- !! -- comment out below to use old module specific web services
