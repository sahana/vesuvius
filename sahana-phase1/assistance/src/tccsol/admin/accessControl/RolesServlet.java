  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
  {
       ...
       try
       {
         //populating the bean with request data
          ...
          {
              if (access.hasAccess(2, logb.getRoleId(), "UPDATE", logb.getRoleName(), "Update/Delete") == false && access.hasAccess(2, logb.getRoleId(), "DELETE", logb.getRoleName(), "Update/Delete") == false)
              {
          ...
          {
              if (access.hasAccess(2, logb.getRoleId(), ptype, logb.getRoleName(), pname) == false)
              {
                 bean.setMessages(access.getMessages());
                 request.setAttribute("messages", bean.getMessages());
                 if (url.equals(arUrl))
          ...
          if (cAct.equals("Add Users"))
          {
              if (bean.getUsers().size() == 0)
              {
                  ...
                  v.add("Employee Name");
                  lbean.setTitles(v);
                  lbean.setSqlStat("select USERNAME, EMPID, concat(concat(FIRSTNAME, ' '), LASTNAME) "
                    + "from TBLPERSONALINFORMATION p, TBLUSERS u where p.EMPNIC = u.EMPNIC "
                    + "and USERNAME not in (select nvl(USERNAME, 'null') from TBLUSERROLES "
                    + "where ROLEID = " + bean.getRoleId() + ") order by EMPID");
           ...
           if (cAct.equals("Remove Users"))
           {
              if (bean.getUsers().size() == 0)
              {
                  ...
                  lbean.setSqlStat("select USERNAME, EMPID, concat(concat(FIRSTNAME, ' '), LASTNAME) "
                    + "from TBLPERSONALINFORMATION p, TBLUSERS u where p.EMPNIC = u.EMPNIC "
                    + "and USERNAME in (select USERNAME from TBLUSERROLES where "
                    + "ROLEID = " + bean.getRoleId() + ") order by EMPID");
