...
public class AccessPermissionServlet extends HttpServlet
{
  ...
  {
       AccessPermissionBean bean = null;
       ...
       try
       {
         //populating the bean with request data
          bean = new AccessPermissionBean();
          session = request.getSession();

          url = request.getParameter("url").trim();
          cAct = request.getParameter("callAction").trim();

          addUrl1 = "/admin/accessControl/AccessPermission1.jsp";
          addUrl2 = "/admin/accessControl/AccessPermission2.jsp";
          ...
          //Modulue Id 3 - Access Permissions
          ...
          if (st == 0)
            ...
            session.setAttribute("AccessPermissionBean", bean);
          ...
          if (btstat == 1)
          {
            if (access.hasAccess(3, logb.getRoleId(), ptype, logb.getRoleName(), pname) == false)
            {
          ...
          if (cAct.equals("Insert"))
          {
              ...
              for (int i=0; i<numL; i++) {
                  if (request.getParameter("perm"+i) != null)
                    bean.getPermissions().add(request.getParameter("perm"+i));
                  else
                    bean.getPermissions().add("N");
          ...
          if (cAct.equals("Select From List"))
          {
                ...
                if (bean.getSelMode().equalsIgnoreCase("R"))
                {
                  ...
                  v.add("Module Name");
                  lsbean.setTitles(v);
                  lsbean.setSqlStat("select MODULEID, MODULENAME from "
                    + "TBLACCESSMODULES order by MODULEID");
                  lsbean.setType("System Modules");
                }
